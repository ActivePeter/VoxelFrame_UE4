...
namespace VF
{
	namespace _Block
	{
			...
			Type::Array<int32>& indices)
		{
			...
			Type::Array<Type::Vec3F> poses;
			info.blockMesh->pushOneFaceVerticesAndIndices_selfPos(dir, poses, indices, vertexPoses.Num());
			//加上网格以及方块的坐标，转换到世界坐标上
			for (auto& i : poses)
			{
				//i *= VF_WorldScale;
				i += Type::Vec3F(
					chunkMeshPos.X + blockx,
					chunkMeshPos.Y + blocky ,
					chunkMeshPos.Z + blockz );
				//swap xy
				auto y = i.Y;
				i.Y =i.Z;
				i.Z = y;
