...
#define __born_entity_switch(__EntityType__)\
	...
	EntityRef EntityManager::born_entity(ServerEntityId seid, EntityType type, VFVec3F&& vf_pos)
	{
		...
		case EntityType::T_Player:
		{
			auto ptr =
				...
				->SpawnActor<AVF_EntityPlayerCpp>(entity_class[int(type)], vf_pos, FRotator::ZeroRotator, param);
		...
		for (auto& entity : entities)
		{
			...
			else//其余entity
			{
				...
				auto ref = born_entity(entity.entity_id(), EntityType::T_Player,
					VFVec3F(entity.x(), entity.y(), entity.z()));
				if (ref.base_ptr)
				{
